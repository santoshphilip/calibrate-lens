"""
Do a projecton for Neil's lens
shade the uncalibrated areas
"""

from PIL import Image, ImageDraw, ImageChops
import pytest
import spathprojection
from datetime import datetime
import math
from pytest_helpers import almostequal
import sunpositions


def circle2box(center_x, center_y, radius):
    """return box from circle data"""
    bbox = (center_x - radius, center_y - radius, center_x + radius, center_y + radius)
    return bbox


@pytest.mark.parametrize(
    "center_x, center_y, radius, bbox",
    [
        (100, 100, 50, (50, 50, 150, 150)),  # center_x, center_y, radius, bbox
    ],
)
def test_circle2box(center_x, center_y, radius, bbox):
    """docstring for fname"""
    result = circle2box(center_x, center_y, radius)
    assert result == bbox


def drawconcentricrings(draw, circle_r):
    """Draw concentric rings of altitiude lines"""
    #     circle_r = 1500
    cx, cy = circle_r, circle_r
    # draw outer circle of the diagram
    bbox = circle2box(cx, cy, circle_r)
    draw.ellipse(bbox, outline="black", width=9)
    for alt in range(0, 90, 10):
        alt_r = spathprojection.neilslens(circle_r, alt)
        bbox = circle2box(cx, cy, alt_r)
        draw.ellipse(bbox, outline=(199, 199, 199), width=4)


def drawazimuthlines(draw, circle_r):
    """draw the azimuth lines"""
    azms_alts = [
        ((a1, 0), (a2, 0)) for a1, a2 in zip(range(0, 180, 30), range(180, 360, 30))
    ]
    lines = []
    for (azm1, alt1), (azm2, alt2) in azms_alts:
        p1 = azmalt2pillowcoords(circle_r, azm1, alt1)
        p2 = azmalt2pillowcoords(circle_r, azm2, alt2)
        lines.append((p1, p2))
    for line in lines:
        draw.line(line, fill=(199, 199, 199), width=4)


def drawanazimuthcircle(draw, alt, circle_r, outline=None, width=None, fill=None):
    """draw a single azimuth circle.
    used to show range of accuracy"""
    if not outline:
        outline = "black"
    if not width:
        width = 9
    cx, cy = circle_r, circle_r
    alt_r = spathprojection.neilslens(circle_r, alt)
    bbox = circle2box(cx, cy, alt_r)
    draw.ellipse(bbox, outline=outline, width=width, fill=fill)


def main(width, height):
    image = Image.new("RGB", (width, height), "white")
    draw = ImageDraw.Draw(image)

    # draw concentric circles for altitude angles
    # and radial azimuth lines
    circle_r = 1500
    drawconcentricrings(draw, circle_r)
    drawazimuthlines(draw, circle_r)
    lat = 41.51606330662579
    lon = 81.39308916058894
    tz = 5.5

    for thehour in range(1, 24):
        # sunposs = sunpositions.analemasunpos(thehour, lat, lon, tz)
        sunposs = sunpositions.smooth_analemasunpos(thehour, lat, lon, tz)
        xys = []
        for _, azm, alt in sunposs:
            pillowx, pillowy = azmalt2pillowcoords(circle_r, azm, alt)
            #             # draw the point on projection.
            #             bbox = circle2box(pillowx, pillowy, 9)
            #             draw.ellipse(bbox, outline=(199, 199, 199), width=1, fill="black")
            xys.append((pillowx, pillowy))
        draw.line(xys, fill="blue", width=6)

    for themonth in range(6, 13):
        thedate = datetime(2025, themonth, 21, 0)
        # sunposs = sunpositions.hourlysunpos(thedate, lat, lon, tz)
        sunposs = sunpositions.smooth_hourlysunpos(thedate, lat, lon, tz)

        xys = []
        for _, azm, alt in sunposs:
            pillowx, pillowy = azmalt2pillowcoords(circle_r, azm, alt)
            #             # draw the point on projection.
            #             bbox = circle2box(pillowx, pillowy, 9)
            #             draw.ellipse(bbox, outline=(199, 199, 199), width=1, fill="black")
            xys.append((pillowx, pillowy))
        draw.line(xys, fill="blue", width=6)

    image = accuracyrange(image, circle_r, 58.586, 15.654)

    image.save("n.png")


def accuracyrange(image, circle_r, above, below):
    """give a transparent shade to the areas not calibrated"""
    # draw range of accuracy

    # from trans7.py
    alt = above
    image = image.convert("RGBA")

    overlay = Image.new("RGBA", image.size, (255, 255, 255, 0))
    od = ImageDraw.Draw(overlay)
    drawanazimuthcircle(
        od,
        alt,
        circle_r,
        outline=(255, 255, 0, 100),
        width=None,
        fill=(255, 255, 0, 100),
    )

    alt = below
    drawanazimuthcircle(od, alt, circle_r, outline=(255, 255, 0, 100), width=None)

    img = Image.alpha_composite(image, overlay)

    # from trans8.py

    # Circle centers & radii
    #     r_outer = circle_r
    #     r_inner = 80

    # --- Outer circle mask ---
    mask_outer = Image.new("RGBA", img.size, (255, 255, 255, 0))
    d1 = ImageDraw.Draw(mask_outer)
    alt = 0
    drawanazimuthcircle(
        d1,
        alt,
        circle_r,
        outline=(255, 255, 0, 100),
        width=None,
        fill=(255, 255, 0, 100),
    )

    # --- Inner circle mask ---
    mask_inner = Image.new("RGBA", img.size, (255, 255, 255, 0))
    d2 = ImageDraw.Draw(mask_inner)
    alt = below
    drawanazimuthcircle(
        d2,
        alt,
        circle_r,
        outline=(255, 255, 0, 100),
        width=None,
        fill=(255, 255, 0, 100),
    )

    # --- Ring mask = outer minus inner ---
    ring_mask = ImageChops.subtract(mask_outer, mask_inner)

    # --- Fill the ring with translucent color ---
    fill_color = (255, 255, 0, 100)
    fill_layer = Image.new("RGBA", img.size, fill_color)
    #     filld = ImageDraw.Draw(fill_layer)
    #     filld.rectangle(((0, 0), fill_layer.size), outline=fill_color,  width=50)
    #     bbox = (0, 0), fill_layer.size
    #     filld.ellipse(bbox, outline=fill_color, width=40, fill=fill_color)
    #     fill_layer.save("cc.png")

    img.paste(fill_layer, mask=ring_mask)

    # Optional: draw outlines

    return img


def azmalt2pillowcoords(circle_r, azm, alt):
    """convert azm, alt to pillow coordinates for drawing"""

    alt_r = spathprojection.neilslens(circle_r, alt)

    # shift into math space and do calcs.
    math_azm = math.radians((azm - 90) * -1)  # x axes is 0 and counterclockise
    # x = rcos(theta)
    # y = rsin(theta)
    cart_x = alt_r * math.cos(math_azm)
    cart_y = alt_r * math.sin(math_azm)

    # Done with math space
    # math space 0,0 is at center of image
    # Pillow 0,0 is on topleft
    pillowx = circle_r + cart_x
    pillowy = circle_r - cart_y
    return pillowx, pillowy


@pytest.mark.parametrize(
    "circle_r, azm, alt, expected",
    [
        (1500, 180, 45, (1500.0, 2250.0)),  # circle_r, azm, alt, expected
        (
            1500,
            45,
            45,
            (2030.3300858899106, 969.6699141100894),
        ),  # circle_r, azm, alt, expected
        (
            1500,
            -45,
            45,
            (969.6699141100894, 969.6699141100893),
        ),  # circle_r, azm, alt, expected
        (1500, 180, 60, (1500.0, 2000.0)),  # circle_r, azm, alt, expected
    ],
)
def test_azmalt2pillowcoords(circle_r, azm, alt, expected):
    """docstring for fname"""
    pass
    result = azmalt2pillowcoords(circle_r, azm, alt)
    assert almostequal(result[0], expected[0])
    assert almostequal(result[1], expected[1])
    assert result == expected


if __name__ == "__main__":
    width, height = 3000, 3000
    main(width, height)
